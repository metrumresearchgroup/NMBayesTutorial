...
setwd(here())
options(mrggsave.dir = "deliv/figure", mrg.script = "applied-sim.R")
...
withr::with_seed(01242022, {
  ...
  map(c(1000,750,500,250), function(xamt){
    simres <- future_map_dfr(
      PKPDmisc::chunk_df(post, ITERATION, .nchunks = 16),.options = opt,
      function(.chunk){
        .chunk = ungroup(.chunk)
        map_dfr(
          .chunk$ITERATION, 
    ...
    saveRDS(simres, file = here(out_data_dir, 
...
simres2 <- readRDS(file = here(out_data_dir, 
                                           'simresPop250.RDS'))
simres3 <- readRDS(file = here(out_data_dir, 
                                         'simresPop500.RDS'))
simres4 <- readRDS(file = here(out_data_dir, 
                                         'simresPop750.RDS'))
simres5 <- readRDS(file = here(out_data_dir, 
                                    'simresPop1000.RDS'))

#Stack the datasets
simres <- bind_rows(simres2, simres3, 
                       simres4, simres5) %>%
  mutate(POP = case_when(DOSE == 250 ~ 'Dose 1',
                         DOSE == 500 ~ 'Dose 2',
                         DOSE == 750 ~ 'Dose 3',
                         DOSE == 1000 ~ 'Dose 4',
                         TRUE ~ 'CHECK'))
...
simressum <- simres %>%
  group_by(POP) %>%
  summarize(qlo = quantile(perc, 0.1),.groups = 'drop'); simressum
...
p1 <- ggplot(simres, aes(x=perc, group = POP)) +
  ...
  geom_vline(xintercept = 90, lty=2, color='steelblue'); p1
